...
(function ($, ctx) {
    ...
    ctx.initQuiz = function () {
        ...
        // Show question(s).
        if (quiz.oneQuestionPerPage()) {
            var index = quiz.getPage() - 1;

            // Calculate xofy.
            questions[index].find('.snax-quiz-question-xofy-y').html( total );

            // Calculate progress.
            var percentage  = (index + 1) / total * 100;
            questions[index].find('.snax-quiz-question-progress-bar').width( percentage + '%' );

            // Show.
            questions[index].find('.snax-quiz-question').removeClass(QUESTION_HIDDEN);
        } else {
...
(function($, ctx) {
    ...
    ctx.Quiz = function(options) {
        ...
        obj.initAnswers = function() {
            ...
            if (currentPage > 1 && options.one_question_per_page && !answeredQuestions) {
